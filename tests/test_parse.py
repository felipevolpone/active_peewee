...
class TestActivePeeewee(unittest.TestCase):
    ...
    def test_eq(self):
        query = Person._parse('by_name_and_age', 'felipe', 30)
        ...
        query = Person._parse('by_name', 'felipe')
    ...
    def test_gt(self):
        query = Person._parse('by_age_gt', 30)

        query_expected = {'age': {'operator': 'gt', 'value': 30}}
        self.assertEqual(query_expected, query)

    def test_ge(self):
        query = Person._parse('by_age_ge', 30)

        query_expected = {'age': {'operator': 'ge', 'value': 30}}
        self.assertEqual(query_expected, query)

    def test_lt(self):
        query = Person._parse('by_age_lt', 30)

        query_expected = {'age': {'operator': 'lt', 'value': 30}}
        self.assertEqual(query_expected, query)

    def test_le(self):
        query = Person._parse('by_age_le', 30)

        query_expected = {'age': {'operator': 'le', 'value': 30}}
        self.assertEqual(query_expected, query)

    def test_or(self):
        query = Person._parse('by_name_or_age', 'felipe', 30)
